...
void pors_gensk(const struct hash *key, const struct address *address, struct pors_sk *sk)
{
    ...
    u8 ssk[66 * PORS_t] = {0};
    u8 rand[66] = {0};
    felem_to_bin66(rand, key->h[0]);
    aesctr256(ssk, rand, iv, 66 * PORS_t);
    int i;
    for (i = 0; i < PORS_t; i++)
    {
        felem a;
        bin66_to_felem(a,ssk+(i*66));
        fpcopy(a, sk->k[i].h[0]);
        fpzero(sk->k[i].h[1]);
    }
...
{
    if (_len < sizeof(struct pors_sign))
...
{
    if (sign1->octolen != sign2->octolen)
        return 1;
    if (hashcmpN(sign1->s.s, sign2->s.s, PORS_k))
        return 1;
    if (hashcmpN(sign1->octopus, sign2->octopus, sign1->octolen))
        return 1;
...
#define STREAMLEN                                                              \
    ...
    hashcpy (&buf[0], rand);
    hashcpy (&buf[1], msg);
    hash_2N_to_N (&seed, &buf[0],&buf[1]);

    u8 seedu8[66]={0};
    bin66_to_felem(seed.h[0],seedu8);

    aesctr256_zeroiv (randstream, seedu8, STREAMLEN);
