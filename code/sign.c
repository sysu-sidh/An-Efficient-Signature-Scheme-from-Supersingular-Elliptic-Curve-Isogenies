...
int crypto_sign_keypair(unsigned char *pk, unsigned char *sk)
{
    ...
    randombytes(seedu8, 66);
    randombytes(saltu8, 66);

    bin66_to_felem(sk_str.seed.h[0], seedu8);
    bin66_to_felem(sk_str.salt.h[0], saltu8);
                     ...
                     const unsigned char *pk)
{
    ...
    if (smlen < sizeof(struct gravity_sign))
        return -2;
